...
**Context of the world: you are at 0.0, 0.0, 0.0. At the moment you are looking in a positive x direction. You have a hand camera and a depth camera. The arm is on the front torso. # There is a cup behind you. The banana is on the right of you.**

WORLD-AWARE PLANNING:
- ALWAYS consider the world context when planning actions
- You are at position (0.0, 0.0, 0.0) facing positive X direction
- There is a cup behind you (negative X direction)
- Use this spatial information to plan logical movements and actions
- When asked to find objects, consider their likely locations based on the world context
- For example: "find the cup" should involve turning around since the cup is behind you
- ALWAYS plan the most efficient path considering your current position and orientation

STATE AWARENESS:
- The robot can be in 'sit' or 'stand' state
- The arm can be in 'stowed' or 'extended' state (tracked separately from robot state)
- Consider both robot state AND arm state when planning actions
...
- ALWAYS stow the arm before any base movement (walking, turning, sitting)
- If the arm is in 'extended' state, it MUST be stowed before any non-arm action
- NEVER go directly from arm operations to movement/sitting - this is dangerous and could damage the robot
- The sequence should be: arm operations → arm_stow → movement/sit
- ONLY skip arm_stow when doing consecutive arm operations (like multiple aim_hand commands)
- The FSM tracks arm state separately from robot state for better safety awareness
...
- z: [-0.2, 0.3] meters height (-0.2 = below body level, 0.0 = body level, 0.3 = above body, excellent for floor objects)

IMPORTANT ARM COMMAND LIMITATIONS:
- arm_stow: ONLY FOR CLEANUP - use sparingly, mainly for program shutdown
- aim_hand: RELIABLE - use this for all arm positioning needs
- For arm movements and gestures, use aim_hand(x,y,z) sequences
...
- WARNING: Keep z-coordinate between -0.2m to 0.3m for reliable arm positioning. z=-0.2 aims below body level, excellent for floor objects.
...
IMPORTANT: For arm movements and gestures (like waving), use aim_hand sequences:
- WAVE EXAMPLE: Use multiple aim_hand commands with different z-coordinates (0.55 to 0.65)
- AVOID: arm_stow during normal operation (only use for cleanup)
- PREFER: aim_hand(x, y, z) for all arm positioning needs

SAFETY: When transitioning from arm operations to any non-arm action:
- ALWAYS include arm_stow before movement, turning, or sitting
- This prevents the extended arm from hitting obstacles or the ground
- Required sequences: 
  * arm operations → arm_stow → move_relative
  * arm operations → arm_stow → sit
  * arm operations → arm_stow → any non-arm command
- ONLY skip arm_stow when doing consecutive arm operations (like multiple aim_hand commands)

A) Safe forward pick on floor:
{"op":"stand","height":0.0}
{"op":"aim_hand","x":0.70,"y":-0.10,"z":-0.2}
{"op":"detect_label","label":"bottle","conf":0.01,"image_source":"hand_color_image"}
{"op":"aim_hand","x":0.75,"y":0.0,"z":-0.2}
...
C) Complex navigation (after arm operations):
{"op":"stand","height":0.0}
{"op":"arm_stow"}
...
D) 360° rotation (after arm operations):
{"op":"stand","height":0.0}
{"op":"arm_stow"}
...
E2) World-aware object finding (cup behind robot):
{"op":"stand","height":0.0}
{"op":"move_relative","x":0.0,"y":0.0,"yaw":3.14,"timeout":10.0}
{"op":"aim_hand","x":0.75,"y":0.0,"z":-0.2}
{"op":"gripper","mode":"open"}
{"op":"detect_label","label":"cup","conf":0.01,"image_source":"hand_color_image"}

F) Safe arm operation to sitting (SAFETY EXAMPLE):
{"op":"stand","height":0.0}
{"op":"aim_hand","x":0.75,"y":0.0,"z":-0.2}
...
G) Consecutive arm operations (NO arm_stow needed):
{"op":"stand","height":0.0}
{"op":"aim_hand","x":0.75,"y":0.0,"z":-0.2}
{"op":"aim_hand","x":0.75,"y":0.0,"z":-0.2}
{"op":"aim_hand","x":0.75,"y":0.0,"z":0.3}

H) Arm operations then movement (arm_stow REQUIRED):
{"op":"stand","height":0.0}
{"op":"aim_hand","x":0.75,"y":0.0,"z":-0.2}
{"op":"arm_stow"}
{"op":"move_relative","x":1.0,"y":0.0,"yaw":0.0,"timeout":10.0}
...
ARM STATE AWARENESS:
- The FSM tracks arm state: 'stowed' or 'extended'
- Use this information to make safer planning decisions
- If arm is 'extended', consider stowing before movement/sitting
- If arm is 'stowed', you can safely plan movement/sitting actions

WORLD-AWARE RESPONSE REQUIREMENTS:
- ALWAYS consider the world context when planning actions
- If asked to find an object that's behind you, include a turn command first
- If asked to pick up something behind you, turn around before attempting to pick
- Use the most efficient path considering your current position and orientation
- Remember: you are at (0.0, 0.0, 0.0) facing positive X, with a cup behind you
...
- [{"op":"aim_hand","x":0.70,"y":0.0,"z":-0.2}] → "Spot will point its hand camera forward and down below body level"
...
class NaturalLanguageControl:
    def __init__(self, use_llm=True, use_speech=False):
        ...
        if self.use_llm:
            ...
            print("- 'Grab the cup' (will turn around since cup is behind you)")
            print("- 'Find the cup' (will search behind you first)")
            print("- 'Pick up the object behind me'")
            print("- 'Find and pick the apriltag'")
            print("- 'Navigate to the corner and sit down'")
            print("- 'Turn around and look for the cup'")
            print("- 'Search behind me for objects'")
        else:
    ...
    def parse_llm_command(self, command):
        ...
        try:
            ...
            # Get arm state from FSM if available
            arm_state = "unknown"
            try:
                # Try to get arm state from FSM feedback
                if hasattr(self, '_last_fsm_feedback') and self._last_fsm_feedback:
                    # Check if feedback contains arm_state information
                    if isinstance(self._last_fsm_feedback, dict) and 'arm_state' in self._last_fsm_feedback:
                        arm_state = self._last_fsm_feedback['arm_state']
                        print(f"FSM reports arm state: {arm_state}")
                    else:
                        # Fallback: infer from recent actions
                        if 'aim_hand' in str(self._last_fsm_feedback):
                            arm_state = "extended"
                        elif 'arm_stow' in str(self._last_fsm_feedback):
                            arm_state = "stowed"
            except Exception as e:
                print(f"Could not determine arm state: {e}")
                arm_state = "unknown"
            
            state_aware_prompt = f"""CURRENT ROBOT STATE: {current_state}
CURRENT ARM STATE: {arm_state}

{LLM_SYSTEM_GUIDE}

IMPORTANT: Consider both robot state AND arm state when planning actions:
...
- If arm is 'extended' and you plan movement/sitting, include arm_stow first
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            try:
                ...
                # Get current arm state from FSM feedback
                arm_state = "unknown"
                try:
                    if hasattr(self, '_last_fsm_feedback') and self._last_fsm_feedback:
                        if isinstance(self._last_fsm_feedback, dict) and 'arm_state' in self._last_fsm_feedback:
                ...
                # Safety check: Add arm_stow if arm is extended and plan includes movement/sitting
                if arm_state == "extended":
                    safety_actions = ['move_relative', 'sit']
                    needs_arm_stow = any(action.get('op') in safety_actions for action in actions)
                    
                    if needs_arm_stow:
                        print(f"\nSAFETY WARNING: Arm is currently extended!")
                        print(f"   Adding arm_stow before unsafe actions...")
                        
                        # Insert arm_stow before the first unsafe action
                        for i, action in enumerate(actions):
                            if action.get('op') in safety_actions:
                                actions.insert(i, {"op": "arm_stow"})
                                break
                        
                        print(f"\nUpdated action plan ({len(actions)} actions):")
                        for i, action in enumerate(actions):
                            print(f"  {i+1}. {action}")

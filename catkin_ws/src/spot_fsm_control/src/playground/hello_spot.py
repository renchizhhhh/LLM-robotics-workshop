...
def hello_spot(config):
    ...
    with bosdyn.client.lease.LeaseKeepAlive(lease_client, must_acquire=True, return_at_exit=True):
        ...
        # print(sources)
        camera_images = [source.name for source in sources]
        print("All camera resources", camera_images)

        for camera in camera_images:
            image_response = image_client.get_image_from_sources([camera])
            # _maybe_display_image(image_response[0].shot.image)
            if config.save or config.save_path is not None:
                _maybe_save_image(image_response[0].shot.image, config.save_path, camera)
...
def _maybe_save_image(image, path, camera):
    ...
    try:
        ...
        return
    name = f"hello-spot-img_{camera}.jpg"
    ...
    try:
        logger.info("The image is of type {}".format(type(image)))
        logger.info("The image has the following definitions {}".format(vars(image)))
        image = Image.open(io.BytesIO(image.data))
        image.save(name)
    except Exception as exc:
        logger = bosdyn.client.util.get_logger()
...
def main(argv):
    ...
    parser.add_argument(
        '--save-path', default="./image_captures/", nargs='?', help=

...
def hello_arm(config):
    ...
    with bosdyn.client.lease.LeaseKeepAlive(lease_client, must_acquire=True, return_at_exit=True):
        ...
        robot.logger.info('Moving arm to hold pose.')

        # Wait until the arm arrives at the goal.
        block_until_arm_arrives_with_prints(robot, command_client, cmd_id, config)

        input("Press Enter to continue...")

        # Move the arm to a different position
        hand_ewrt_flat_body.x = 1.0
        ...
        # Send the request
        cmd_id = command_client.robot_command(command)
        robot.logger.info('Moving arm to releasing pose.')

        # Wait until the arm arrives at the goal.
        # Note: here we use the helper function provided by robot_command.
        block_until_arm_arrives_with_prints(command_client, cmd_id, config)
...
def block_until_arm_arrives_with_prints(robot, command_client, cmd_id, config):
    ...
    while True:
        ...
        if config.verbose:
            rospy.loginfo('Distance to go: %.2f meters, %.2f radians',
...
if __name__ == '__main__':
    # If running in a ROS environment, initialize as a ROS node and read parameters from
    # the private namespace. Otherwise fall back to the existing CLI behavior.
    if (rospy is not None) and ('ROS_MASTER_URI' in os.environ or 'ROS_NAMESPACE' in os.environ):
        rospy.init_node('pose_grasp_node', anonymous=True)

        # Read parameters from the node's private namespace with sensible defaults.
        ...
        try:
            hello_arm(options)
        except Exception:  # Keep same exception handling behavior as before
            logger = bosdyn.client.util.get_logger()
            logger.exception('Threw an exception')
            sys.exit(1)
    else:

...
class SpotControlInterface(ManipulatorFunctions):
    ...
    def pick_from_pixel(self, image_source, x, y, top_down=True, auto_walk=False, timeout=25.0):
        ...
        try:
            # Create pick request
            pick_request = manipulation_api_pb2.PickObjectInImageRequest(
                pixel_xy=geometry_pb2.Vec2(x=x, y=y),
                transforms_snapshot_for_camera=self.robot.get_frame_tree_snapshot(),
                frame_name_image_sensor=image_source,
                pick_pose_frame_name=ODOM_FRAME_NAME,
                top_down=top_down,
                auto_walk=auto_walk
            )
            
            # Send request
            cmd_id = self.manipulation_api_client.pick_object_in_image(pick_request)

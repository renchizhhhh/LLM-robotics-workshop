...
from spot_hololens_llm_interface.data_logger import DataLogger
...
class FsmNode:
    ...
    def _execute_command(self, command):
        ...
        try:
            ...
            # Special handling for arm_stow - use direct approach that works during shutdown
            if op == 'arm_stow':
                print("Using direct arm_stow approach (same as shutdown)")
                try:
                    # Call arm_stow directly on the interface (bypassing FSM complexity)
                    success = self.spot_interface.arm_stow()
                    if success:
                        # Update FSM arm state
                        self.state_machine._arm_state = 'stowed'
                        self.state_machine._last_arm_action = 'arm_stow'
                        print("Direct arm_stow succeeded (like during shutdown)")
                    else:
                        print("Direct arm_stow returned False")
                except Exception as e:
                    print(f"Direct arm_stow failed: {e}")
                    success = False
            else:
                # Enqueue action in state machine for all other commands
    ...
    def _publish_feedback(self, status, operation, message=""):
        ...
        # Add arm state information if available
        if hasattr(self, 'state_machine') and self.state_machine:
            feedback["arm_state"] = self.state_machine.get_arm_state()
            feedback["robot_state"] = str(self.state_machine.current_state)
...
def main():
    ...
    try:
        ...
        # Simple user mode toggle like V1 - set this manually
        ...
        if DUMMY_MODE:
            ...
            try:
                ...
                # Take lease forcefully (like V1) - use TakeLease to grab already claimed lease
                lease_client = robot.ensure_client(bosdyn.client.lease.LeaseClient.default_service_name)
                try:
                    # First try normal acquisition
                    lease = lease_client.acquire()
                    print("Lease acquired normally")
                except Exception as e:
                    if "ResourceAlreadyClaimedError" in str(e):
                        print("Lease already claimed, taking it forcefully...")
                        # Use TakeLease to forcefully grab the lease
                        lease = lease_client.take_lease()
                        print("Lease taken forcefully")
                    else:
                        raise e
                ...
                class DummySpotInterface:
                    def __init__(self):
                        self.last_detection = None
                    
                    def stand(self, height=0.0):
                        print(f"DUMMY: Standing with height {height}")
                        return True
                    
                    def sit(self):
                        print("DUMMY: Sitting down")
                        return True
                    
                    def move_relative(self, x, y, yaw, timeout=10.0):
                        print(f"DUMMY: Moving relative x={x}, y={y}, yaw={yaw}")
                        return True
                    
                    def arm_stow(self):
                        print("DUMMY: Stowing arm")
                        return True
                    
                    def aim_hand(self, x, y, z):
                        print(f"DUMMY: Aiming hand at x={x}, y={y}, z={z}")
                        return True
                    

                    
                    def detect_label(self, label, provider='ultralytics', conf=0.01, image_source='hand_color_image'):
                        print(f"DUMMY: Detecting label {label}")
                        self.last_detection = {'label': label, 'confidence': conf, 'bbox': [100, 100, 200, 200], 'center': [150, 150]}
                        return True
                    
                    def pick_from_pixel(self, image_source, x, y, top_down=True, auto_walk=False, timeout=25.0):
                        print(f"DUMMY: Picking from pixel ({x}, {y})")
                        return True
                    
                    def grasp_body_point(self, x, y, z, approach=0.12, lift=0.15):
                        print(f"DUMMY: Grasping body point ({x}, {y}, {z})")
                        return True
                    
                    def pick_apriltag(self, tag_id=None, z_offset=0.20, approach=0.12, lift=0.15):
                        print(f"DUMMY: Picking AprilTag {tag_id}")
                        return True
                
                spot_interface = DummySpotInterface()
        
        # Create FSM node
        is_dummy = hasattr(spot_interface, '__class__') and 'DummySpotInterface' in spot_interface.__class__.__name__
        fsm_node = FsmNode(hostname, dummy_mode=is_dummy)
        
        # Initialize with the interface (dummy or real)
        fsm_node.spot_interface = spot_interface
        fsm_node.state_machine = SpotStateMachine(spot_interface, dummy_mode=is_dummy)

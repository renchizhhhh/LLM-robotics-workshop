...
class SpotControlInterface(ManipulatorFunctions):
    ...
    def gripper(self, close_or_open):
        print(f"Gripper command: {close_or_open}")
        try:
            if close_or_open == "open":
                ...
                # Close gripper to 50% strength (0.5 = 50% closed, 1.0 = fully open, 0.0 = fully closed)
                cmd = RobotCommandBuilder.claw_gripper_open_fraction_command(0.5)
                self.command_client.robot_command(cmd)
                print("✅ Gripper close command sent (fraction: 0.5 = 50% strength)")
            else:
                print(f"⚠️  Unknown gripper command: {close_or_open}")
                return False
            self.prev_close_or_open = close_or_open
            return True
        except Exception as e:
            print(f"❌ Error sending gripper command '{close_or_open}': {e}")
            return False
